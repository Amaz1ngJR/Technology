...
|类型	|映射关系|	端口转换	|公网IP节省	|外部可访问性|
|-------|--------|--------|-------------|------|
|**静态NAT**|	1:1|	❌ 否|	❌ 否	|  ✅ 可（如服务器）  |
|**动态NAT**	|N:M 临时（IP池））|	❌ 否|	❌ 部分节省（仍需多IP）	| ⚠️ 临时可|
|**【最常用】PAT/NAPT**|N:1（IP+端口）|	✅ 是	|✅ 是（一个IP支持多设备）|❌ 默认不可（除非端口映射）|
...
# 传输层
## UDP
<img width="754" height="467" alt="image" src="https://github.com/user-attachments/assets/6c254b13-74ef-4df2-872a-76afb75dedf1" />
...
RST（重置）：该位为 1 时，表示 TCP 连接中出现异常必须强制断开连接
SYN（建立）：该位为 1 时，表示希望建立连接，并在其「序列号」的字段进行序列号初始值的设定
FIN（终止）：该位为 1 时，表示今后不会再有数据发送，希望断开连接。当通信结束希望断开连接时，通信双方的主机之间就可以相互交换 FIN 位为 1 的 TCP 段
```
...
### 端口问题
理论最大端口数：65536（16位端口号） TCP和UDP可以使用同一个端口
<img width="744" height="939" alt="image" src="https://github.com/user-attachments/assets/3606e5af-1931-4031-a89d-5a75becaff75" />
...
<img width="735" height="358" alt="image" src="https://github.com/user-attachments/assets/03ce07f1-edda-4d2b-a9ea-7ae1f7611ca8" />
...
## MQTT
MQTT (Message Queuing Telemetry Transport) 是一种轻量级的发布/订阅式消息传输协议，基于 TCP/IP 协议，设计用于低带宽、高延迟、不稳定的网络环境，广泛应用于物联网（IoT）场景。默认端口 1883（非加密）、8883（TLS加密）

### 核心概念
- **发布者（Publisher）**：发送消息的客户端
- **订阅者（Subscriber）**：接收消息的客户端
- **代理服务器（Broker）**：消息中转服务器，负责接收、过滤、分发消息（如 Mosquitto、EMQX）
- **主题（Topic）**：消息的分类标签，采用层级结构（如 `home/livingroom/temperature`）

### 工作原理
```
1. 订阅者连接到 Broker，订阅感兴趣的主题（Topic）
2. 发布者连接到 Broker，向指定主题发布消息
3. Broker 将消息分发给所有订阅了该主题的订阅者
```

### 主题通配符
- `+`：单层通配符，匹配一个层级
  - 例如：`home/+/temperature` 可匹配 `home/livingroom/temperature`、`home/bedroom/temperature`
- `#`：多层通配符，只能放在末尾，匹配多个层级
  - 例如：`home/#` 可匹配 `home/livingroom/temperature`、`home/bedroom/light/status`

### QoS（服务质量等级）
| QoS 级别 | 名称 | 描述 | 适用场景 |
|---------|------|------|---------|
| **QoS 0** | 至多一次 | 消息最多发送一次，可能丢失，无确认 | 传感器数据频繁上报，允许偶尔丢失 |
| **QoS 1** | 至少一次 | 保证消息到达，但可能重复，需 PUBACK 确认 | 重要通知、命令下发 |
| **QoS 2** | 恰好一次 | 四次握手确保消息不丢失不重复（PUBLISH → PUBREC → PUBREL → PUBCOMP） | 支付、计费等严格场景 |
...
| PUBLISH | 双向 | 发布消息 |
| PUBACK | 双向 | QoS 1 的确认 |
| PUBREC | 双向 | QoS 2 的接收确认（第一步） |
| PUBREL | 双向 | QoS 2 的释放（第二步） |
| PUBCOMP | 双向 | QoS 2 的完成（第三步） |
| SUBSCRIBE | 客户端 → Broker | 订阅主题 |
| SUBACK | Broker → 客户端 | 订阅确认 |
| UNSUBSCRIBE | 客户端 → Broker | 取消订阅 |
| UNSUBACK | Broker → 客户端 | 取消订阅确认 |
| PINGREQ | 客户端 → Broker | 心跳请求（保持连接） |
| PINGRESP | Broker → 客户端 | 心跳响应 |
| DISCONNECT | 客户端 → Broker | 断开连接通知 |

### 保持连接（Keep Alive）
客户端在 CONNECT 报文中指定 Keep Alive 时间间隔（秒），在此时间内如果没有消息交互，客户端需要发送 PINGREQ，Broker 回复 PINGRESP。如果 1.5 倍 Keep Alive 时间内未收到任何消息，Broker 会认为客户端离线并断开连接

### 遗嘱消息（Last Will）
客户端连接时可设置遗嘱消息（Will Message），当客户端异常断开（非正常 DISCONNECT）时，Broker 会自动向指定主题发布该遗嘱消息，用于通知其他客户端该设备离线

### 持久会话（Clean Session）
- **Clean Session = 0**：Broker 保存客户端的订阅信息和未传递的 QoS 1/2 消息，客户端重连后可继续接收
- **Clean Session = 1**：客户端断开后，Broker 清除所有会话信息

### 保留消息（Retained Message）
发布消息时设置 Retain 标志，Broker 会保留该主题的最后一条消息。新订阅者订阅该主题时，会立即收到这条保留消息（常用于设备状态同步）

### MQTT vs HTTP
| 特性 | MQTT | HTTP |
|------|------|------|
| 连接方式 | 长连接（持久） | 短连接（请求-响应） |
| 通信模式 | 发布/订阅 | 请求/响应 |
| 消息推送 | 支持（Broker 主动推送） | 不支持（需轮询或 WebSocket） |
| 开销 | 极小（固定头部 2 字节起） | 较大（HTTP 头部通常几百字节） |
| 适用场景 | 物联网、实时通信、低功耗设备 | Web 应用、API 接口 |

### 实际应用场景
- 智能家居：温湿度传感器上报数据、智能灯开关控制
- 车联网：车辆位置上报、远程控制
- 工业物联网：设备状态监控、生产数据采集
- 即时通讯：轻量级聊天应用（如 Facebook Messenger 早期使用 MQTT）