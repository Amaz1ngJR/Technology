...
void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {//按下ESC键退出
        glfwSetWindowShouldClose(window, true);
    }
}
...
int main() {
	...
	*/
    ...
    std::cout << "OpenGL Context: " 
              << glGetString(GL_VERSION) << std::endl;// 输出 OpenGL 版本

    // 主循环
    while (!glfwWindowShouldClose(window)) {
        // 渲染
        // 设置清屏颜色并清除颜色缓冲
        glClearColor(0.2f, 0.3f, 0.5f, 1.0f);//状态设置函数
        glClear(GL_COLOR_BUFFER_BIT);//状态使用函数 清空屏幕的颜色缓冲

        // TODO: 绘制你的图形（三角形、正方形等）
