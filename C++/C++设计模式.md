...
2.单一职责原则：
一个类或者模块应有且仅有一个导致其变更的原因 一个类应该专注于一个核心职责 只做一件事
如果一个类承担了多个职责，当需求发生变化时，可能会导致类的多个部分都需要修改，增加了耦合性和维护难度
3.里氏替换原则：
子类应当能够替换掉它们的基类，在程序中任何使用基类的地方，都可以透明地使用子类对象而无需更改原有逻辑
这意味着子类必须保持与基类行为的一致性，并且不应破坏基类的封装性和契约
4.依赖倒置原则：
面向接口编程，依赖于抽象而不依赖于具体。写代码时用到具体类时，不与具体类交互，而是与具体类的上层接口交互
5.接口隔离原则：
每个接口中不存在子类用不到却必须实现的方法，如果不然，就要将接口拆分。使用多个隔离的接口，比使用单个接口要好
6.迪米特原则（最少知道原则）：
一个类对自己依赖的类知道的越少越好。无论被依赖的类多复杂，都应该将逻辑封装在方法的内部，通过public方法提供给外部
这样当被依赖的类变化时，才能最小影响该类 降低系统的耦合度，提高模块的独立性和可理解性
7.合成复用原则：
在软件复用时，应优先考虑使用对象组合而不是类继承。继承容易导致类的层次结构复杂化，增加系统的耦合度，
而合成则能更好地实现“部分-整体”关系，保持类的独立性和可扩展性
```
# 创建型模式
